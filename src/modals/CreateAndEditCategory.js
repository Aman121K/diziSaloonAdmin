...
const CreateAndEditCategory = ({ openModal, setOpenModal, getAllCategory, category, id, setId }) => {
    ...
    const createCategory = () => {
        ...
        return postData1(Constants.END_POINT.CREATE_CATEGORY, formData)
            .then((res) => {
                if (res.success) {
                    getAllCategory();
                    setOpenModal(false);
        ...
        >
            ...
            <div className="mt-2">
                ...
                <span className="p-label flex justify-content-between">
                    ...
                    {isEdit ? <img src={img ? img : Constants.BASE_URL + form?.categoryImage} alt="" width="100px" height="85px" /> : <p>{form?.categoryImage.name}</p>}
                </span>
